...
import { fetchCommentsPostId } from '@/db/queries'
...
export default async function Comments({ slug, id }: Props) {
  const { comments } = await fetchCommentsPostId(id)
          ...
          >
            ...
            {comment?.children.map(child => {
              return (
                <article
                  key={child.id}
                  className='ml-2 border-l border-neutral-800/100 pl-0.5 sm:ml-4'
                >
                  ...
                  {child?.children.map(subchild => {
                    return (
                      <article
                        key={subchild.id}
                        className='ml-4 border-l border-neutral-800/80 pl-0.5 sm:ml-6'
                      >
